...
class HashTable {
    ...
    public void delete(int ele) {  // function to delete
    ...
    private static int nextPrime(int n) {  // Function to generate next prime number >= n
        if (n % 2 == 0)
            n++;
        for (; !isPrime(n); n += 2);
        return n;
    ...
    private static boolean isPrime(int n) {  // Function to check if given number is prime
        if (n == 2 || n == 3)
            return true;
        if (n == 1 || n % 2 == 0)
            return false;
    ...
    public void printTable() {  // function to print hash table
        System.out.print("\nHash Table = ");
...
public class Main {  // Changed class name to Main for compatibility with your environment
